...
def compute_factor_panel(series: PriceSeries, window: int = 20) -> dict[str, list[float | None]]:
    """Compute the value of every factor at every bar.

    This is the input the ranking layer needs. factor[bar] uses only bars
    [0..bar], so there's no lookahead — this is the same guarantee that
    signal_at gives the backtester.

    factor_panel["mom_20"][t] == compute_features(truncated_to(t))["mom_20"]
    for all valid t, via the lookahead pin test.

    Args:
        series: Price data
        window: Default lookback window for features

    Returns:
        dict[factor_name, list[float | None]] with one entry per bar.
    """
    n = len(series.candles)
    if n < 2:
        return {k: [] for k in {kk for v in FEATURE_GROUPS.values() for kk in v}}

    # Get feature names from the groups
    all_keys = [k for keys in FEATURE_GROUPS.values() for k in keys]
    panel: dict[str, list[float | None]] = {k: [None] * n for k in all_keys}

    # Compute features at each bar using only bars [0..bar]
    # For efficiency at large n, start from window: earlier bars have too few
    # candles and produce all Nones anyway for most features.
    # We still fill [0..window] positions with None (the default).
    for bar in range(window, n):
        truncated = PriceSeries(
            asset=series.asset,
            interval=series.interval,
            candles=series.candles[: bar + 1],
        )
        feat = compute_features(truncated, window=window)
        for k in all_keys:
            panel[k][bar] = feat.get(k)
